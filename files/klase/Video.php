...
class Video {
	...
	public function isInProgress($username) {
		$query = $this->conn->prepare("SELECT * FROM videoprogress WHERE videoId=:videoId AND username=:username AND finished=0");
	...
	public function hasSeen($username) {
		$query = $this->conn->prepare("SELECT * FROM videoprogress WHERE videoId=:videoId AND username=:username AND finished=1");
