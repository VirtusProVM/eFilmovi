	...
	class Category {
		...
		public function showAllCategories() {
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)) {
				$html .= $this->getCategory($row, null, true, true);
			}
		...
		public function showCategory($categoryID, $title = null) {
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)) {
				$html .= $this->getCategory($row, $title, true, true);
			}
			return $html . "</div>";
		}
		...
		public function showTVShowsCategory() {
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)) {
				$html .= $this->getCategory($row, null, true, false);
			}
			return $html . "</div>";
		}
		...
		public function showMoviesCategory() {
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)) {
				$html .= $this->getCategory($row, null, false, true);
			}
			return $html . "</div>";
		}


		private function getCategory($sqlData, $title, $tvShows, $movies) {
			...
			if($tvShows && $movies) {
				...
				$entities = EntityProvider::getTVShowsEntities($this->conn, $categoryId, 30);
			} else {
			...
			foreach ($entities as $entity) {
			 	$entitiesHtml .= $previewProvider->createEntityProvider($entity);
			}
